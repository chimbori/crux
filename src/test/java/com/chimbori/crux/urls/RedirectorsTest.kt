...
class RedirectorsTest {
  ...
  fun testIsLikelyType() {
    assertEquals(true, "http://example.com/video.mp4".toHttpUrlOrNull()?.isLikelyVideo())
    assertEquals(true, "http://example.com/video.mpg".toHttpUrlOrNull()?.isLikelyVideo())
    assertEquals(true, "http://example.com/video.avi".toHttpUrlOrNull()?.isLikelyVideo())
    assertEquals(false, "http://example.com/test.txt".toHttpUrlOrNull()?.isLikelyVideo())
    assertEquals(false, "http://example.com/test.tmp".toHttpUrlOrNull()?.isLikelyVideo())
    assertEquals(false, "http://example.com/test.log".toHttpUrlOrNull()?.isLikelyVideo())
  }
  ...
  fun testNoOpRedirects() {
    val exampleNoRedirects = "http://example.com".toHttpUrlOrNull()?.resolveRedirects()
  ...
  fun testRedirects() {
    assertEquals("http://www.bet.com/collegemarketingreps",
        "http://www.facebook.com/l.php?u=http%3A%2F%2Fwww.bet.com%2Fcollegemarketingreps&h=42263"
            .toHttpUrlOrNull()?.resolveRedirects().toString())
  ...
  fun testGoogleRedirectors() {
    assertEquals("https://www.facebook.com/permalink.php?id=111262459538815&story_fbid=534292497235807",
